...
const instance = await startInstance({
  ...
  searches: new WorkspaceSearchService({ async searchWorkspace(memberId, workspaceId, query) { return memberId === browserMemberId && workspaceId === browserWorkspaceId ? { status: "found", results: query.q === "discussion" ? [{ id: browserDiscussions[0].id, kind: "discussion", title: "Keep this release context", href: `/app/notes/${noteId}/discussions` }] : query.q === "task" ? [{ id: task.id, kind: "task", title: `${task.key} · ${task.title}`, excerpt: "Canonical Task planning result", href: `/app/projects/${projectId}/tasks/${task.key}`, projectId, author: "Browser Member", assignee: "Browser Member", status: task.status.name, occurredAt: "2026-08-22T10:00:00.000Z" }] : [{ id: noteId, kind: "note", title: "Release collaboration plan", excerpt: `Matched ${query.q}`, href: `/app/notes/${noteId}`, author: "Browser Member", status: "active", occurredAt: "2026-08-21T10:00:00.000Z" }] } : { status: "forbidden" }; } }),

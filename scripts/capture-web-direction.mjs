...
const starterAvailable = await starterGuide.isVisible({ timeout: 5_000 }).catch(() => false);
if (starterAvailable) {
  ...
  await desktop.screenshot({ path: `${output}/02-starter-workspace.png` });
...
await desktop.screenshot({ path: `${output}/03-note-tree-editor.png` });
...
await desktop.screenshot({ path: `${output}/04-links-backlinks.png` });
...
await narrow.screenshot({ path: `${output}/11-responsive-note.png` });

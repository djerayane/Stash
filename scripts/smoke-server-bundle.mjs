...
function isolatedInvocation(launcher, arguments_, traceRoot) {
  ...
  if (process.platform === "linux") { const hidden = ["/usr/bin/docker", "/usr/local/bin/docker", "/usr/bin/podman", "/usr/local/bin/podman"].filter(existsSync); return { command: "/usr/bin/sudo", arguments: ["--non-interactive", "/usr/bin/bwrap", "--die-with-parent", "--new-session", "--ro-bind", "/", "/", "--dev-bind", "/dev", "/dev", "--proc", "/proc", "--tmpfs", "/run", "--bind", traceRoot, traceRoot, ...hidden.flatMap((path) => ["--bind", "/dev/null", path]), "/usr/bin/setpriv", `--reuid=${process.getuid()}`, `--regid=${process.getgid()}`, "--clear-groups", "/usr/bin/strace", "-ff", "-o", join(traceRoot, `syscalls-${randomUUID()}`), "-e", "trace=process,network,file", launcher, ...arguments_] }; }
...
async function migrateFixture({ bundle, launcher, environment, postgresUrl, dataDirectory, extraction, email, password }) {
  ...
  try { await source.database.createFirstOrganizationOwner({ organizationId: expected.organizationId, organizationName: "Bundle Migration", ownerId: expected.ownerId,
    ...
    await source.database.identityAccessRepositories().createWorkspace({ id: expected.workspaceId, name: "Rich Bundle Workspace", owner: { type: "organization", id: expected.organizationId }, createdByMemberId: expected.ownerId }, { localAccountId: expected.ownerId, displayName: "Bundle Owner" });

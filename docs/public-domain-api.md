...
Notifications are derived from attributed `stash.activity.v1` records by trusted domain adapters inside the Instance; there is deliberately no public endpoint that accepts caller-authored notification events or Activity records. Assignment Activity and its notification are produced atomically by Task changes. Direct mentions, assignments, requested reviews, failed Automations, and followed changes remain relevant even when ordinary Project activity is muted. For followed changes, `all` includes every change, `followed` includes only events identified as followed by the producing domain adapter, and `muted` suppresses them. Member-initiated self-authored Activity is suppressed; a failed Automation is system-caused and notifies its configuring Member even though that Member remains its attribution actor. `GET /api/v1/notifications` returns the current permission-filtered inbox, `GET /api/v1/notifications/digest` atomically claims unread entries in the current UTC daily or Monday-based weekly window so retrying the same delivery does not duplicate it, and `POST /api/v1/notifications/{id}/read` records read state. `GET` and `PUT /api/v1/projects/{projectId}/notification-settings` expose `activity` (`all`, `followed`, or `muted`), `digest` (`off`, `daily`, or `weekly`), and optional local quiet hours. Entries created during quiet hours carry `delivery: "quiet_hours"`, allowing delivery workers and clients to defer interruptions without hiding the event from the feed.

Discussion messages use `<@member-uuid>` for stable direct mentions. The Instance derives recipients from the committed message, restricts delivery to Members who can access its target Project or Workspace-wide Note, suppresses self-mentions and duplicates, and records attributed Activity and inbox delivery in the same transaction. Workspace-scoped mentions have no `projectId` and use immediate delivery because Project notification settings do not apply to them.

Notification producers are internal adapters invoked by canonical domain mutations. Failed Automation execution now derives its configuring Member from the persisted recipe, records one attributed Activity for each Automation, Signal, and Task combination, and reuses that identity when delivery is retried. Until another underlying mutation exists, the Instance does not expose a stand-alone source command: an authenticated Member still cannot manufacture a review request, Automation failure, followed change, attributed Activity, or notification.

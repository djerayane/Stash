...
Non-assignment notification producers are internal adapters invoked by the canonical Discussion, review, Automation, and follow mutations as those capabilities are implemented. Until an underlying mutation exists, the Instance does not expose a stand-alone source command: an authenticated Member still cannot manufacture a mention, review request, Automation failure, followed change, attributed Activity, or notification.

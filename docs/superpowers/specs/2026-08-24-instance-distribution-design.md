...
6. Standalone migration preserves usable encrypted authentication, recovery, and integration state through explicit operator-controlled key preservation or re-encryption, rolls back safely on failure, and never exposes either master key in an artifact or log.

...
From a fresh checkout, `docker compose up -d` builds and starts PostgreSQL and one Stash application container without requiring an `.env` file or exported variables. The Instance becomes healthy and serves the web application at `http://localhost:3000`.
...
The embedded engine is an adapter behind the same permission-aware repositories and transaction boundaries as PostgreSQL. It must pass the shared behavioral contract for migrations, transactions, search, synchronization, durable jobs, backup/restore, and Portable Workspace Export. Unsupported PostgreSQL behavior fails during build or preflight rather than degrading a feature silently. A documented migration command exports a consistent standalone Instance and imports it into an empty, compatible external-PostgreSQL Instance with identities, history, configuration, Attachments, and checksums preserved.
...
4. A canonical semantic-version tag whose ref and version have no publication collision passes the common quality checks and tag-only release preflight before publishing multi-architecture GHCR images, checksummed standalone bundles, signed Android artifacts, and an IPA only when iOS signing is configured; every publish job directly depends on that gate.
...
6. Release documentation lets an operator select a path and reach the web interface without inspecting source code.

...
describe("ProjectNotificationsPage", () => {
  it("does not start settings motion when reduced motion is requested", async () => {
    vi.spyOn(gsap, "from");
    vi.stubGlobal("matchMedia", vi.fn(() => ({ matches: true, addEventListener() {}, removeEventListener() {} })));
    vi.stubGlobal("fetch", vi.fn(async (path: string) => new Response(JSON.stringify(path.endsWith("/follow")
      ? { followed: false } : { settings: { activity: "followed", digest: "off" } }), { status: 200 })));
    render(<QueryClientProvider client={new QueryClient()}><MemoryRouter initialEntries={["/app/projects/44444444-4444-4444-8444-444444444444/notifications"]}><Routes>

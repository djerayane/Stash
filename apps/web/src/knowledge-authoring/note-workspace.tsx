...
export function NoteWorkspace({ noteId, token, children, fetcher = globalThis.fetch }: {
  ...
  return <div className={`${styles.noteWorkspace} ${drawerOpen ? styles.drawerIsOpen : ""}`}>
    <section aria-label="Note workspace controls" className={styles.workspaceBar}>
      ...
      <div className={styles.workspaceActions}><Link to={`/app/notes/${noteId}/history`}>View history</Link>{removedState ? <button aria-label="Restore Note branch" disabled={restore.isPending} type="button" onClick={() => restore.mutate()}>Restore branch</button> : <>

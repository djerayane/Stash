...
export function CollectionViewControls({ collection, definition, editable, onChange, onAddRequiredProperty }: {
  ...
  const choose = (presentation: ViewPresentation) => { const requirement = presentationRequirement(collection, presentation);
  ...
  return <div className={styles.viewArea}><div className={styles.presentation} role="group" aria-label="Collection presentation">
    {presentations.map((entry) => <button type="button" key={entry.value} aria-pressed={definition.presentation === entry.value}
      onClick={() => choose(entry.value)}>{entry.label}</button>)}
    <button type="button" aria-expanded={viewOpen} onClick={() => setViewOpen((open) => !open)}>View{active ? ` · ${active}` : ""}</button>
  </div>{requirement && requested ? <div className={styles.requirement} role="status"><p>{requested === "calendar"
    ? "Calendar needs a date property." : "Board needs a select property."}</p><Button pending={pending} type="button" onClick={() => { setPending(true);
      void onAddRequiredProperty(requirement.propertyType, requested).then(() => setRequested(undefined)).finally(() => setPending(false)); }}>{requirement.actionLabel}</Button>
    ...
    {viewOpen ? <div className={styles.viewControls} aria-label="View settings">
      <Field label="Filter by"><select aria-label="Filter by" value={filter?.propertyId ?? ""} onChange={(event) => onChange({ ...definition,
      ...
      {filter ? <><Field label="Filter condition"><select aria-label="Filter condition" value={filter.operator} onChange={(event) => {
      ...
      <Field label="Sort by"><select aria-label="Sort by" value={definition.sorts[0]?.propertyId ?? ""} onChange={(event) => onChange({ ...definition,
        sorts: event.target.value ? [{ propertyId: event.target.value, direction: "ascending" }] : [] })}><option value="">Record order</option>
        {collection.properties.map((property) => <option key={property.id} value={property.id}>{property.name}</option>)}</select></Field>
      <Field label="Group by"><select aria-label="Group by" value={definition.groupBy ?? ""} onChange={(event) => onChange({ ...definition,
      ...
      <Field label="Density"><select aria-label="Density" value={String(definition.layout.density ?? "comfortable")} onChange={(event) => onChange({ ...definition,
      ...
      <fieldset><legend>Visible properties</legend>{collection.properties.map((property) => <label key={property.id}><input type="checkbox"

...
export function ContextDrawer({ context, onClose, token, fetcher = globalThis.fetch }: { context: NoteContextData; onClose(): void; token: string; fetcher?: typeof fetch }) {
  ...
  useGSAP(() => {
    if (!drawerRef.current || window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) return;
    gsap.fromTo(drawerRef.current, { opacity: .86, x: 12 }, { opacity: 1, x: 0, duration: .22, ease: "power2.out", clearProps: "all" });
  }, { scope: drawerRef });
  ...
  return <aside className={styles.drawer} aria-label="Note context" ref={drawerRef}>
    <header><h2>{context.breadcrumbs.at(-1)?.title ?? "Note"}</h2><IconButton label="Close Note context" onClick={onClose}><span aria-hidden="true">×</span></IconButton></header>
    ...
    <div className={styles.drawerBody}>
      <section {...panel("links")} hidden={tab !== "links"} role="tabpanel">{context.access === "edit" ? <form className={styles.linkForm} onSubmit={(event: FormEvent) => { event.preventDefault(); createLink.mutate(); }}>
        <h3>Link another Note</h3><Field label="Target Note"><select required value={targetNoteId} onChange={(event) => setTargetNoteId(event.target.value)}><option value="">Choose a Note</option>
          {tree.data?.nodes.filter(({ id }) => id !== context.noteId).map((note) => <option key={note.id} value={note.id}>{note.title}</option>)}</select></Field>
        <Field label="Relationship type"><input maxLength={80} placeholder="supports, contradicts…" value={relationshipType} onChange={(event) => setRelationshipType(event.target.value)} /></Field>
        <Button pending={createLink.isPending} pendingLabel="Creating Note link" type="submit">Create Note link</Button>{createLink.isError ? <p role="alert">{createLink.error.message}</p> : null}

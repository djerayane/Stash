...
export function SearchPage({ workspaceId, token, fetcher = globalThis.fetch }: CoreProps) {
  ...
  return <div className={styles.page}>
    <Header title="Search" lede="Find permitted Notes, Tasks, Projects, and activity across this Workspace." />
    <Field className={styles.searchQuery} label="Search this view"><input autoFocus type="search" value={term} placeholder="Search Notes, Tasks, and Projects" onChange={(event) => updateFilter("q", event.target.value)} /></Field>
    <FilterDisclosure className={styles.filterDisclosure}>
      <section className={styles.searchFilters} aria-label="Search filters">
        <Field label="Project"><input value={params.get("projectId") || ""} onChange={(event) => updateFilter("projectId", event.target.value)} /></Field>
        <Field label="Object type"><select value={params.get("object") || ""} onChange={(event) => updateFilter("object", event.target.value)}><option value="">All objects</option>{["note", "task", "discussion", "file", "label", "member", "development"].map((kind) => <option key={kind} value={kind}>{words(kind)}</option>)}</select></Field>
        <Field label="Author"><input value={params.get("author") || ""} onChange={(event) => updateFilter("author", event.target.value)} /></Field>
        <Field label="Assignee"><input value={params.get("assignee") || ""} onChange={(event) => updateFilter("assignee", event.target.value)} /></Field>
        <Field label="Status"><input value={params.get("status") || ""} onChange={(event) => updateFilter("status", event.target.value)} /></Field>
        <Field label="From"><input type="date" value={params.get("from") || ""} onChange={(event) => updateFilter("from", event.target.value)} /></Field>
        <Field label="To"><input type="date" value={params.get("to") || ""} onChange={(event) => updateFilter("to", event.target.value)} /></Field>
        <Button type="button" variant="secondary" onClick={clearFilters}>Clear filters</Button>
      </section>
    </FilterDisclosure>
    {!term ? <Empty title="Search your Workspace" body="Use the search field above to find Notes, Tasks, Discussions, files, labels, Members, and development data." /> : query.isPending ? <Loading /> : query.isError ? <Failure error={query.error} retry={() => void query.refetch()} recovery="Your search and filters are preserved. Try the same query again." /> : query.data.results.length ? <><aside aria-label="Permitted search summary"><p>{query.data.total} permitted {query.data.total === 1 ? "match" : "matches"}</p><ul>{query.data.facets.kinds.map((facet) => <li key={facet.value}>{words(facet.value)} {facet.count}</li>)}</ul></aside><div className={styles.searchResults} ref={resultsRef}>{query.data.results.map((result) => result.href ? <Link key={`${result.kind}-${result.id}`} to={result.href}>{resultBody(result)}</Link> : <article key={`${result.kind}-${result.id}`}>{resultBody(result)}</article>)}</div></> : <Empty title="No permitted matches" body="Try another phrase or remove a filter." />}
  </div>;
}

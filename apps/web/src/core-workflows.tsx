...
export function BoardsPage({ token, fetcher = globalThis.fetch }: Omit<CoreProps, "workspaceId">) {
  ...
  return <div className={styles.page}><Header title={board.data?.board.name || "Workflow Boards"} lede="Boards are views over the Project Workflow; Tasks remain the canonical work." />{!boardId ? boards.isPending ? <Loading /> : boards.isError ? <Failure error={boards.error} retry={() => void boards.refetch()} /> : <div className={styles.boardIndex}>{boards.data.boards.map((item) => <Link key={item.id} to={`/app/projects/${projectId}/boards/${item.id}`}>{item.name}</Link>)}</div> : board.isPending ? <Loading /> : board.isError ? <Failure error={board.error} retry={() => void board.refetch()} /> : <div className={styles.board}>{board.data.columns.map((column) => <section key={column.id}><h2>{column.name}<span>{column.tasks.length}</span></h2>{column.tasks.map((task) => <article key={task.id}><Link to={`/app/projects/${projectId}/tasks/${task.key}`}>{task.key}</Link><strong>{task.title}</strong><label>Move Task<span className={styles.srOnly}> {task.key}</span><select data-task-key={task.key} disabled={move.isPending} value={column.id} onChange={(event) => move.mutate({ task, statusId: event.target.value })}>{board.data.columns.filter((target) => !target.archived).map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}</select></label></article>)}</section>)}</div>}<p className={styles.srOnly} role="status">{moveStatus}</p>{move.isError ? <p role="alert">{move.error.message}</p> : null}</div>;
}

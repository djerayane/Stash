...
export function TaskAutomations({ projectId, taskKey, fetcher = globalThis.fetch, token = "" }: Props) {
  ...
  const configureErrorRef = useRef<HTMLParagraphElement>(null);
  const reverseErrorRef = useRef<HTMLParagraphElement>(null);
  ...
  return <section className={styles.section} aria-labelledby="automation-title">
    ...
    {query.data ? <>
      ...
      </article>)}{reverse.isError ? <p className={styles.inlineError} role="alert" tabIndex={-1} ref={reverseErrorRef}>{reverse.error.message}{reverse.variables ? <button type="button" onClick={() => reverse.mutate(reverse.variables!)}>Try again</button> : null}</p> : null}</div> : null}
    </> : null}
  </section>;
}

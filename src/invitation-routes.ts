...
export function invitationRoutes(service: InvitationService, accessResolver: MemberAccessResolver): HttpRoute {
  return {
    ...
    async handle(request, response, url) {
      ...
      try {
        organizationId = invitationMatch ? decodePathSegment(invitationMatch[1]!) : undefined;
        projectId = projectMatch ? decodePathSegment(projectMatch[1]!) : undefined;
      } catch (error) {
        if (error instanceof InvalidInvitationInput) {
      ...
      try {
        ...
        if (error instanceof InvalidInvitationInput) invalidInput(response);
...
function decodePathSegment(value: string): string {
  try { return decodeURIComponent(value); } catch { throw new InvalidInvitationInput(); }
}

function invalidInput(response: Parameters<typeof json>[0]): void {
  json(response, 422, { error: "invalid_input", message: "Invitation and Project values must be valid." });
}

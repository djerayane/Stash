...
export function repositoryConnectionRoutes(service: RepositoryConnectionService, access: MemberAccessResolver): HttpRoute {
  return { matches: (_request, url) => path.test(url.pathname) || projectPath.test(url.pathname) || verifyPath.test(url.pathname), async handle(request, response, url) {
    ...
    try {
      const match = url.pathname.match(path) ?? url.pathname.match(projectPath) ?? url.pathname.match(verifyPath)!;

...
export interface RepositoryConnectionRepository {
  ...
  repairRepositoryConnection?(actorId: string, organizationId: string, connectionId: string): Promise<"repaired" | "not_found" | "forbidden">;
}
...
export class RepositoryConnectionService {
  ...
  async repair(actorId: string, organizationId: string, connectionId: string) { if (!isUuid(connectionId) || !this.repository.repairRepositoryConnection) throw new InvalidRepositoryConnectionInput(); return this.repository.repairRepositoryConnection(actorId, organizationId, connectionId); }

...
export interface RepositoryConnectionRecord { id: string; organizationId: string; provider: "github"; installationId: number; repositoryId: string; repositoryUrl: string; createdByMemberId: string; createdByAttribution: "recorded" | "inferred-during-upgrade"; projectIds: string[] }
export interface PortableRepositoryConnectionProjection { schema: "stash.repository-connection.v1"; id: string; provider: "github"; repositoryUrl: string; organization: { localOrganizationId: string; displayName: string }; createdBy: { localAccountId: string; displayName: string; attribution: "recorded" | "inferred-during-upgrade" }; projectIds: string[] }
...
export class RepositoryConnectionService {
  ...
  async connect(actorId: string, organizationId: string, value: unknown): Promise<{ created: boolean; connection: RepositoryConnectionProjection }> {
    ...
    const result = await this.repository.createRepositoryConnection(actorId, { id: randomUUID(), organizationId, provider: "github", ...inspected, createdByMemberId: actorId, createdByAttribution: "recorded", projectIds: [] });
  ...
  async verify(organizationId: string, connectionId: string): Promise<void> { if (!isUuid(connectionId)) throw new InvalidRepositoryConnectionInput(); const record = await this.repository.findRepositoryConnectionById(organizationId, connectionId); if (!record) throw new InvalidRepositoryConnectionInput(); try { await this.github.verifyRepository(record); } catch { throw new GitHubRepositoryUnavailable(); } }
}
function project(record: RepositoryConnectionRecord): RepositoryConnectionProjection { return { id: record.id, organizationId: record.organizationId, provider: record.provider, repositoryId: record.repositoryId, repositoryUrl: record.repositoryUrl, projectIds: [...record.projectIds] }; }
function isInput(value: unknown): value is GitHubRepositorySelection { if (!value || typeof value !== "object" || Array.isArray(value)) return false; const input = value as Record<string, unknown>; return Object.keys(input).length === 3 && Number.isSafeInteger(input.installationId) && (input.installationId as number) > 0 && typeof input.owner === "string" && /^[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})$/.test(input.owner) && typeof input.name === "string" && input.name.length > 0 && input.name.length <= 100 && !/[\s/]/.test(input.name); }

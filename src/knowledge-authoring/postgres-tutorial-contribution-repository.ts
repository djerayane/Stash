...
import type { Collection, CollectionRepository, CollectionViewResult, StarterKnowledgeSetup, TutorialContribution, TutorialContributionRepository, ViewBlock } from "./collections.js";
...
export class PostgresTutorialContributionRepository implements TutorialContributionRepository, CollectionRepository, NoteTreeImpactInspector, NoteTreeBranchLifecycle {
  ...
  async createCanonicalViewBlock(memberId: string, view: CanonicalViewBlock): Promise<
    ...
    return this.kernel.transaction(async (client) => {
      ...
      if (view.definition.source.kind === "collection") {
        const source = await client.query(`SELECT 1 FROM stash_collections WHERE id=$1 AND workspace_id=$2`,
          [view.definition.source.collectionId, view.workspaceId]);
  ...
  async updateCanonicalViewBlock(memberId: string, viewId: string, definition: ViewDefinition): Promise<
    ...
    return this.kernel.transaction(async (client) => {
      ...
      if (definition.source.kind === "collection") {
        if (!(await client.query("SELECT 1 FROM stash_collections WHERE id=$1 AND workspace_id=$2", [definition.source.collectionId, workspaceId])).rowCount)

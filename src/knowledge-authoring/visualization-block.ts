...
export interface VisualizationBlockRepository {
  save(memberId: string, noteId: string, definition: VisualizationDefinition, expectedRevision: number | undefined, idempotencyKey: string): Promise<
    { status: "saved"; block: SavedVisualizationBlock } | { status: "not_found" | "idempotency_conflict" }
    | { status: "changed"; block: SavedVisualizationBlock }>;
...
export class VisualizationBlockService {
  ...
  async save(memberId: string, noteId: string, value: unknown, expectedRevision: number | undefined, idempotencyKey: unknown) {
    if (!uuid.test(noteId) || expectedRevision !== undefined && (!Number.isInteger(expectedRevision) || expectedRevision < 1)
    ...
    return this.repository.save(memberId, noteId, definition, expectedRevision, idempotencyKey);
  }

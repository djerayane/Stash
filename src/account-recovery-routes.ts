...
import type { AuthenticatedMember } from "./password-auth.js";

export interface MemberAccessResolver { resolve(authorization: string | undefined): Promise<AuthenticatedMember | undefined> }

export function accountRecoveryRoute(service: AccountRecoveryService, members: MemberAccessResolver): HttpRoute {
  ...
  return { matches: (_request, url) => paths.has(url.pathname), async handle(request, response, url) {
    try {
      ...
      const member = await members.resolve(request.headers.authorization);

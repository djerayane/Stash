...
export function noteEditorRoute(): HttpRoute {
  return {
    ...
    async handle(_request, response) {
      ...
      fetch('/api/notes/'+noteId,{headers:{authorization:'Bearer '+token()}}).then(async response=>{const body=await response.json();if(!response.ok)throw new Error(body.message);baseDocument=body.document;render(body.document);noteRevision=body.revision;editor.contentEditable='true';editor.setAttribute('aria-busy','false');document.querySelectorAll('button').forEach(button=>button.disabled=false);announce('Ready','ready');move('.toolbar',{from:{opacity:0,y:-8},to:{opacity:1,y:0}});move(editor,{from:{opacity:0,scale:.99},to:{opacity:1,scale:1},duration:.36})}).catch(error=>{announce(error.message||'The Note could not be loaded.','error')});

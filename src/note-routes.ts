...
export function noteEditorRoute(): HttpRoute {
  return {
    ...
    async handle(_request, response) {
      ...
      const render=document=>{editor.replaceChildren();for(const block of document.blocks){const element=document.createElement(block.type==='heading'?'h'+block.level:block.type==='quote'?'blockquote':block.type==='code'?'pre':block.type==='bullet'||block.type==='check'?'li':'p');element.dataset.blockType=block.type;element.dataset.blockKey=block.blockKey;if(block.id)element.dataset.blockId=block.id;if(block.type==='check'){const checkbox=document.createElement('input');checkbox.type='checkbox';checkbox.checked=block.checked;checkbox.setAttribute('aria-label','Checklist state');checkbox.contentEditable='false';element.append(checkbox)}if(block.type==='code'){element.dataset.language=block.language||'';element.textContent=block.text}else for(const span of block.content)addSpan(element,span);editor.append(element);const tasks=linkedTasks.filter(task=>task.sourceBlock.blockId===block.id);if(tasks.length){const links=document.createElement('aside');links.className='task-links';links.dataset.taskLinks='';links.contentEditable='false';links.setAttribute('aria-label','Linked Tasks');for(const task of tasks){const item=document.createElement('span');item.className='task-link';const key=document.createElement('span');key.className='task-link-key';key.textContent=task.key;const title=document.createElement('span');title.textContent=task.title;const state=document.createElement('span');state.className='task-link-status';state.textContent=task.status.name;item.append(key,title,state);links.append(item)}editor.append(links)}}};

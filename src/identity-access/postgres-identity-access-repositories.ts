...
export class PostgresIdentityAccessRepositories implements PasswordAuthRepository, AccountRegistrationRepository,
  ...
  async savePasskey(record:PasskeyRecord):Promise<void>{await this.prepareRecovery();await this.kernel.query(
  ...
  [record.credentialId,record.accountId,this.secrets.encrypt(record.publicKey),record.counter,record.transports??null,record.createdAt]);}
  async findPasskey(credentialId:string):Promise<PasskeyRecord|undefined>{await this.prepareRecovery();const row=(await this.kernel.query<{
    credential_id:string;account_id:string;public_key:string;signature_counter:number;transports:string[]|null;created_at:Date|string}>(
    "SELECT credential_id,account_id,public_key,signature_counter,transports,created_at FROM stash_passkeys WHERE credential_id=$1",[credentialId])).rows[0];
    return row?{credentialId:row.credential_id,accountId:row.account_id,publicKey:this.secrets.decrypt(row.public_key),counter:row.signature_counter,
      ...(row.transports?{transports:row.transports}:{}),createdAt:new Date(row.created_at).toISOString()}:undefined;}
  async updatePasskeyCounterAndCreateSession(credentialId:string,previousCounter:number,newCounter:number,session:SessionRecord):Promise<boolean>{
    await this.prepareRecovery();return this.kernel.controlledTransaction(async(client)=>{const result=await client.query(
      "UPDATE stash_passkeys SET signature_counter=$3 WHERE credential_id=$1 AND signature_counter=$2",[credentialId,previousCounter,newCounter]);
      if(result.rowCount!==1)return {commit:false,value:false};await this.insertSession(client,session);return {commit:true,value:true};});}
  async replaceRecoveryCodes(accountId:string,records:RecoveryCodeRecord[]):Promise<void>{await this.prepareRecovery();await this.kernel.transaction(async(client)=>{
    await client.query("DELETE FROM stash_recovery_codes WHERE account_id=$1",[accountId]);for(const record of records)await client.query(
      "INSERT INTO stash_recovery_codes(account_id,code_lookup,protected_secret) VALUES($1,$2,$3)",[accountId,this.secrets.blindIndex(record.lookup),record.protectedSecret]);});}
  async consumeRecoveryCodeAndCreateSession(accountId:string,lookup:string,session?:SessionRecord):Promise<boolean>{await this.prepareRecovery();
    return this.kernel.controlledTransaction(async(client)=>{const result=await client.query(
      "DELETE FROM stash_recovery_codes WHERE account_id=$1 AND code_lookup=$2",[accountId,this.secrets.blindIndex(lookup)]);
      if(result.rowCount!==1||!session)return {commit:false,value:false};await this.insertSession(client,session);return {commit:true,value:true};});}
  async enqueueEmailRecovery(job:EmailRecoveryDeliveryJob):Promise<void>{await this.prepareRecovery();await this.kernel.query(
    "INSERT INTO stash_email_recovery_delivery_jobs(id,protected_delivery,created_at) VALUES($1,$2,$3)",[job.id,job.protectedDelivery,job.createdAt]);}
  async findEmailRecoveryAccount(lookup:string,now:string):Promise<string|undefined>{await this.prepareRecovery();return (await this.kernel.query<{account_id:string}>(
    "SELECT account_id FROM stash_email_recoveries WHERE token_lookup=$1 AND expires_at>$2",[this.secrets.blindIndex(lookup),now])).rows[0]?.account_id;}
  async consumeEmailRecoveryAndCreateSession(lookup:string,now:string,session:SessionRecord):Promise<boolean>{await this.prepareRecovery();
    return this.kernel.controlledTransaction(async(client)=>{const result=await client.query(
      "DELETE FROM stash_email_recoveries WHERE token_lookup=$1 AND account_id=$2 AND expires_at>$3",[this.secrets.blindIndex(lookup),session.accountId,now]);
      if(result.rowCount!==1)return {commit:false,value:false};await this.insertSession(client,session);return {commit:true,value:true};});}
  async claimEmailRecoveryDelivery(owner:string,leaseUntil:string):Promise<ClaimedEmailRecoveryDelivery|undefined>{await this.prepareRecovery();
    return this.kernel.controlledTransaction(async(client)=>{const row=(await client.query<{id:string;protected_delivery:string;created_at:Date|string;claim_version:number}>(
      "SELECT id,protected_delivery,created_at,claim_version FROM stash_email_recovery_delivery_jobs WHERE available_at<=NOW() AND (lease_until IS NULL OR lease_until<=NOW()) ORDER BY created_at LIMIT 1 FOR UPDATE SKIP LOCKED")).rows[0];
      if(!row)return {commit:false,value:undefined};const version=Number(row.claim_version)+1;await client.query(
        "UPDATE stash_email_recovery_delivery_jobs SET claim_owner=$2,claim_version=$3,lease_until=$4 WHERE id=$1",[row.id,owner,version,leaseUntil]);
      return {commit:true,value:{job:{id:row.id,protectedDelivery:row.protected_delivery,createdAt:new Date(row.created_at).toISOString()},claim:{jobId:row.id,owner,version}}};});}
  async renewEmailRecoveryDelivery(claim:EmailRecoveryDeliveryClaim,leaseUntil:string):Promise<boolean>{return (await this.kernel.query(
    "UPDATE stash_email_recovery_delivery_jobs SET lease_until=$4 WHERE id=$1 AND claim_owner=$2 AND claim_version=$3",[claim.jobId,claim.owner,claim.version,leaseUntil])).rowCount===1;}
  async completeEmailRecoveryDelivery(claim:EmailRecoveryDeliveryClaim,activation?:EmailRecoveryRecord):Promise<boolean>{await this.prepareRecovery();
    return this.kernel.controlledTransaction(async(client)=>{const deleted=await client.query(
      "DELETE FROM stash_email_recovery_delivery_jobs WHERE id=$1 AND claim_owner=$2 AND claim_version=$3",[claim.jobId,claim.owner,claim.version]);
      if(deleted.rowCount!==1)return {commit:false,value:false};if(activation)await client.query(
        "INSERT INTO stash_email_recoveries(token_lookup,account_id,protected_secret,expires_at) VALUES($1,$2,$3,$4)",
      [this.secrets.blindIndex(activation.tokenLookup),activation.accountId,activation.protectedSecret,activation.expiresAt]);return {commit:true,value:true};});}
  ...
  async prepareRecovery(client:PostgresQueryable=this.kernel):Promise<void>{await this.dependencies.prepareRegistration(client);await client.query(`
    ...
    CREATE TABLE IF NOT EXISTS stash_recovery_codes(account_id UUID NOT NULL REFERENCES stash_accounts(id) ON DELETE CASCADE,code_lookup TEXT NOT NULL,
      protected_secret TEXT NOT NULL,PRIMARY KEY(account_id,code_lookup));
    CREATE TABLE IF NOT EXISTS stash_email_recoveries(token_lookup TEXT PRIMARY KEY,account_id UUID NOT NULL REFERENCES stash_accounts(id) ON DELETE CASCADE,
      protected_secret TEXT NOT NULL,expires_at TIMESTAMPTZ NOT NULL);
    CREATE TABLE IF NOT EXISTS stash_email_recovery_delivery_jobs(id UUID PRIMARY KEY,protected_delivery TEXT NOT NULL,created_at TIMESTAMPTZ NOT NULL,

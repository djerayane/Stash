import type { MobileCapture, MobileSyncMutation, MobileSyncResult } from "../../src/mobile-capture-client";
...
export function presentMobileSyncResult(result: MobileSyncResult, outbox: Array<MobileCapture | MobileSyncMutation>): string {
  ...
  if (result.status === "attention_required" && result.error === "conflicts_preserved")
    return "Every conflicting contribution was preserved. Review it in Stash to resolve the conflict.";

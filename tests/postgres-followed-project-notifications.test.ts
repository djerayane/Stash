...
describe("PostgreSQL followed Project notifications", { skip: databaseUrl ? false : "STASH_TEST_DATABASE_URL is not configured" }, () => {
  it("delivers canonical Project Note Activity and rechecks recipient membership", async () => {
    ...
    try {
      ...
      await sql.query("INSERT INTO stash_accounts(id,name,email,password_hash) VALUES($1,'Grace Hopper','grace-follow@example.test','test-only')", [recipientId]);
      ...
      if (created.status === "created") {
        const blockKey = created.note.document.blocks[0]!.id!; const tasks = new TaskService(database, database);

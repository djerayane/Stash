...
describe("PostgreSQL GitHub Signal acceptance", { skip: databaseUrl ? false : "STASH_TEST_DATABASE_URL is not configured" }, () => {
  it("records and notifies each exact failed Automation execution once", async () => {
    ...
    try {
      ...
      const workspaces = new WorkspaceProjectService(database.identityAccessRepositories()); const notes = new NoteService(database.knowledgeAuthoringRepositories()); const tasks = new TaskService(database.workPlanningRepositories(), database.identityAccessRepositories());

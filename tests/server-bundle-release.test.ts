...
describe("self-contained Instance bundle packaging", () => {
  ...
  test("keeps pull requests non-publishing and gates target bundles plus the release publisher", async () => {
    ...
    assert.match(smoke, /sudo[\s\S]*--non-interactive[\s\S]*bwrap[\s\S]*--tmpfs[\s\S]*\/run[\s\S]*\/dev\/null[\s\S]*setpriv[\s\S]*--reuid[\s\S]*--regid[\s\S]*--clear-groups/); assert.match(smoke, /netsh/); assert.match(smoke, /dir=out[\s\S]*action=block/);
    assert.match(smoke, /program="\$\{runtime\}"/);
    assert.match(smoke, /name="\$\{name\}"/);

...
export interface ProductSettingsApi {
  ...
  importMarkdownWorkspace(memberId:string,archive:Blob):Promise<Record<string,unknown>>;
}
...
export function createProductSettingsApi(options: StashApiClientOptions): ProductSettingsApi {
  ...
  return {
    ...
    importMarkdownWorkspace:(memberId,archive)=>json(send("/api/workspace-imports/markdown",{method:"POST",headers:{"content-type":"application/zip","idempotency-key":crypto.randomUUID(),"x-stash-import-owner-account-id":memberId},body:archive}),jsonObjectResponse),
  };
}

...
export interface ProductSettingsApi {
  ...
  createCustomRole(organizationId: string, input: { name: string; permissions: Array<"create_project"> }): Promise<void>;
  updateCustomRole(organizationId: string, roleId: string, input: { name: string; permissions: Array<"create_project"> }): Promise<void>;
  assignCustomRole(organizationId: string, roleId: string, memberId: string): Promise<void>;
  revokeCustomRole(organizationId: string, roleId: string, memberId: string): Promise<void>;
...
export function createProductSettingsApi(options: StashApiClientOptions): ProductSettingsApi {
  ...
  return {
    ...
    createCustomRole: (organizationId, input) => write(`${root(organizationId)}/roles`, "POST", input),
    updateCustomRole: (organizationId, roleId, input) => write(`${root(organizationId)}/roles/${encodeURIComponent(roleId)}`, "PUT", input),
    assignCustomRole: (organizationId, roleId, memberId) => write(`${root(organizationId)}/roles/${encodeURIComponent(roleId)}/members/${encodeURIComponent(memberId)}`, "PUT", {}),
    revokeCustomRole: async (organizationId, roleId, memberId) => { await send(`${root(organizationId)}/roles/${encodeURIComponent(roleId)}/members/${encodeURIComponent(memberId)}`, { method: "DELETE" }); },

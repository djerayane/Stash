...
export function organizationRolesResponse(value: unknown): ValidationResult<{ roles: OrganizationRoleSummary[] }> { if (!plain(value) || !exact(value, ["roles"]) || !Array.isArray(value.roles) || !value.roles.every((role) => plain(role) && (role.immutable === true
  ? exact(role, ["name", "immutable", "permissions"]) && ["Owner", "Admin", "Member"].includes(String(role.name)) && Array.isArray(role.permissions) && role.permissions.every((permission) => typeof permission === "string")
  : role.immutable === false && exact(role, ["id", "name", "immutable", "permissions", "memberIds"]) && typeof role.id === "string" && typeof role.name === "string" && Array.isArray(role.permissions) && role.permissions.every((permission) => permission === "create_project") && Array.isArray(role.memberIds) && role.memberIds.every((id) => typeof id === "string")))) return { ok: false, message: "Organization Roles response is invalid" }; return { ok: true, value: value as unknown as { roles: OrganizationRoleSummary[] } }; }

...
export function memberLocalizationResponse(value: unknown): ValidationResult<MemberLocalizationSettings> { if (!plain(value) || typeof value.locale !== "string" || typeof value.timeZone !== "string" || !["short", "medium", "long"].includes(String(value.dateFormat)) || !["sunday", "monday", "saturday"].includes(String(value.weekStartsOn))) return { ok: false, message: "Localization response is invalid" }; return { ok: true, value: value as unknown as MemberLocalizationSettings }; }
export function recoveryCodeResponse(value: unknown): ValidationResult<RecoveryCodeResponse> { if (!plain(value) || !Array.isArray(value.codes) || !value.codes.every((code) => typeof code === "string")) return { ok: false, message: "Recovery code response is invalid" }; return { ok: true, value: { codes: value.codes as string[] } }; }
export function invitationResponse(value: unknown): ValidationResult<OrganizationInvitationResponse> { if (!plain(value) || typeof value.token !== "string") return { ok: false, message: "Invitation response is invalid" }; return { ok: true, value: { token: value.token } }; }
export function organizationRolesResponse(value: unknown): ValidationResult<{ roles: OrganizationRoleSummary[] }> { if (!plain(value) || !Array.isArray(value.roles) || !value.roles.every((role) => plain(role) && typeof role.name === "string" && (role.description === undefined || typeof role.description === "string"))) return { ok: false, message: "Organization Roles response is invalid" }; return { ok: true, value: value as unknown as { roles: OrganizationRoleSummary[] } }; }
export function repositoryConnectionsResponse(value: unknown): ValidationResult<{ repositoryConnections: OrganizationRepositoryConnection[] }> { if (!plain(value) || !Array.isArray(value.repositoryConnections) || !value.repositoryConnections.every((connection) => plain(connection) && typeof connection.id === "string" && typeof connection.repositoryUrl === "string" && Array.isArray(connection.projectIds) && connection.projectIds.every((id) => typeof id === "string") && ["organization", "personal"].includes(String(connection.ownership)) && ["active", "degraded"].includes(String(connection.state)))) return { ok: false, message: "Repository Connections response is invalid" }; return { ok: true, value: value as unknown as { repositoryConnections: OrganizationRepositoryConnection[] } }; }
...
export function instanceDiagnosticsResponse(value: unknown): ValidationResult<InstanceDiagnosticsState> { if (!plain(value) || !plain(value.settings) || typeof value.settings.diagnosticSubmissions !== "boolean" || typeof value.settings.crashReportSubmissions !== "boolean" || typeof value.settings.updateChecks !== "boolean" || !Array.isArray(value.pending) || !Array.isArray(value.pendingCrashReports)) return { ok: false, message: "Instance diagnostics response is invalid" }; return { ok: true, value: value as unknown as InstanceDiagnosticsState }; }
